...
public class NoticeServiceImp implements NoticeService {
    ...
    public NoticeServiceImp(ResourceLoader resourceLoader) {
        this.resourceLoader = resourceLoader;
        UPLOAD_DIR2 = "static/notics/";
    }
    ...
    public void insertNotice(NoticeEntity notice, MultipartFile file) {
        try {
            String originalFileName = StringUtils.cleanPath(file.getOriginalFilename());
            notice.setFileName(originalFileName);
            Resource uploadResource = resourceLoader.getResource("classpath:" + UPLOAD_DIR2 + originalFileName);

            // Save file to server
            try (InputStream inputStream = file.getInputStream()) {
                // Here you would save the file from inputStream to your desired location
                // For example, you could use Files.copy(inputStream, Paths.get(uploadResource.getURI()));
            }
...
//    }
  ...
  public List<NoticeEntity> fetchNotice() {
    ...
    @Override
    public ResponseEntity<Resource> openFileInNewTab(String fileName) {
        Resource resource = resourceLoader.getResource("classpath:" + UPLOAD_DIR2 + fileName);
        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + fileName);
        String fileExtension = StringUtils.getFilenameExtension(fileName);
        MediaType contentType = CONTENT_TYPE_MAP.getOrDefault(fileExtension.toLowerCase(), MediaType.APPLICATION_OCTET_STREAM);
